...
function buildMongoFiles(baseDir, appname) {
    ...
    fs.copyFileSync(`${baseDir}/services/mongo.${language}`, `${appname}/src/services/mongo-service.${language}`);
}

function buildRabbitMqFiles(baseDir, appname) {
    fs.mkdirSync(`${appname}/src/subscribers`);
    fs.copyFileSync(`${baseDir}/services/rabbitmq.${language}`, `${appname}/src/services/rabbitmq-service.${language}`);
}
